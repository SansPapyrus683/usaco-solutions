...
segmentTree = MaxSegTree(len(heights))
for v, h in enumerate(heights):
    segmentTree.set(v, h)

totalCrowded = 0
for i in range(1, cowNum - 1):  # the first cow and last cow don't really matter
    farthestLeft = bisect_left(positions, positions[i] - distThreshold)
    farthestRight = bisect_right(positions, positions[i] + distThreshold) - 1
    totalCrowded += segmentTree.max(i + 1, farthestRight + 1) >= heights[i] * 2 and \
                    segmentTree.max(farthestLeft, i) >= heights[i] * 2

print(totalCrowded)
print(totalCrowded, file=open('crowded.out', 'w'))
