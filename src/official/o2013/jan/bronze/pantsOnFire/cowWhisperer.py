    ...
    def consistent(self):
        ...
        for c in self.cows:
            ...
            while frontier:
                ...
                thisConfig = hypothetical[curr]
                if thisConfig == INVALID:
                    raise RuntimeError("this shouldn't happen lol")

                for other, good in self.testimonies.get(curr, []):
                    supposed = thisConfig if good else (0 if thisConfig == 1 else 1)
...
with open('truth.in') as read:
    cowNum, statementNum = [int(i) for i in read.readline().split()]
