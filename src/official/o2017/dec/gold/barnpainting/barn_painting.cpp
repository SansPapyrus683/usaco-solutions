...
class Farm {
   ...
   private:
    ...
    long long paint_poss(int at, int parent, int root_color) {
        ...
        if (painted_alr[at] != -1 && painted_alr[at] != root_color) {
            return 0;
        }
        ...
        for (int n : neighbors[at]) {
            if (n != parent) {
                ...
                for (int c = 0; c < COLOR_NUM; c++) {
                    // can't paint a child the same color as we are now, can we?
                    if (c != root_color) {
   ...
   public:
    ...
    long long paint_poss() {
        long long total = 0;
        for (int c = 0; c < COLOR_NUM; c++) {
            // just arbitrarily root the tree at 0
            total += paint_poss(0, 0, c);
        }
        return total % MOD;
    }
};
