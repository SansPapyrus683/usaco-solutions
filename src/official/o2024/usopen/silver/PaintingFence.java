...
public class PaintingFence {
    public static void main(String[] args) throws IOException {
        ...
        for (int c = 0; c < cowNum; c++) {
            ...
            if (dist < distPref[postNum] - dist) {
                rubNum[data[0][0] + (data[0][1] == 0 ? 0 : 1)]++;
                rubNum[data[1][0] + 1]--;
            } else {
                rubNum[0]++;
                rubNum[data[0][0] + 1]--;
                rubNum[data[1][0] + (data[1][1] == 0 ? 0 : 1)]++;
            }
        }

        int[] ans = new int[postNum];
        for (int i = 0; i < postNum; i++) {
            rubNum[i] += (i > 0 ? rubNum[i - 1] : 0);
            ans[order[i]] = rubNum[i];
        }
