...
if mother.get(first, REDACTED) == mother.get(second, REDACTED + 'asdf'):
    ...
    for c in [first, second]:
        dist = 0
        at = c
        possAncestor = second if c == first else first
        while at in mother:  # go up the family tree
            dist += 1
            if mother[at] == possAncestor:  # right up the ancestor line = mother or grandmother or whatever
                words = ['great' for _ in range(dist - 2)]
                if dist > 1:
                    words.append('grand')
                words.append('mother')
                relation = '%s is the %s of %s' % (possAncestor, '-'.join(words), c)
                break
            # a direct child of a direct ancestor = aunt
            elif possAncestor in children[mother[at]]:
