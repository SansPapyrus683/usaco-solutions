...
combinedSizes = []
for comb in combinations(roomList, 2):  # tries a bunch of room combinations
    currentPair = []
    for w in comb[0][1]:
        if w in comb[1][1]:
            currentPair.append(w)  # since there are many "optimal" walls, we have to take in all of them so no break
    if currentPair:
        currentPair.append(comb[0][0] + comb[1][0])  # only append to valid combinations if it's a thing
